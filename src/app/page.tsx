...
export default function Home() {
  ...
  // ✅ Logout : coupe audio + vide l'UI + purge la session locale + redirige vers page neutre
  const handleLogout = async () => {
    try {
      await stopRecordingAndCleanup(); // coupe micro/lectures si besoin
      await supabase.auth.signOut({ scope: 'global' });
      safeStorage.remove("chatMessages"); // supprime l'historique persistant côté navigateur
      setMessages([]);                    // vide l'affichage du chat
      setIsAuthed(false);
      // évite un éventuel relog via ?code=... / rafraîchissement d'état OAuth
