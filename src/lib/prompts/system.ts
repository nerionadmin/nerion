...
// System prompt builder for Nerion — CLEAN version
// Phases: intro → photo_user → big_five → iri → ecr_r → pvq_40 → complete → default
// NOTE: All photo logic previously inlined as PHOTO_PROMPT has been removed.
//       The photo-user phase now imports its prompt from ./photo/user
...
} from "./psychometry/psychometry_wrapper"; // wrappers = (stimulus: string) => string

import { PHOTO_USER_PROMPT } from "./photo/user";

/* =========================
 * Persona & Global Policies
 * ========================= */
...
- Never mention triggers, systems, storage, or backend logic.
`.trim();
...
/**
 * buildSystemPrompt
 * - "intro"      : welcome + confirmation → emits TriggerPhotoUserTrue to enter photo_user
 * - "photo_user" : self-image intake phase (prompts imported from ./photo/user)
